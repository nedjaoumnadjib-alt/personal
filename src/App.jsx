      ...
      <section id="about" style={styles.section}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, alignItems: "center" }}>
            <div>
              <p style={{ color: "#d0c7ff", fontSize: 16, lineHeight: 1.8, marginBottom: 24 }}>{t.about.p1}</p>
              <p style={{ color: "#d0c7ff", fontSize: 16, lineHeight: 1.8 }}>{t.about.p2}</p>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {t.about.stats.map((stat, i) => (
                <div key={i} style={{ background: "rgba(127,106,248,0.12)", border: "1px solid rgba(127,106,248,0.24)", borderRadius: 14, padding: 24, textAlign: "center" }}>
                  <p style={{ color: "#7f6af8", fontSize: 28, fontWeight: 700, margin: "0 0 8px" }}>{t.about.vals[i]}</p>
                  <p style={{ color: "#d0c7ff", fontSize: 13, margin: 0 }}>{stat}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <div style={styles.card}>
            <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
              <input style={styles.input} placeholder={t.status.placeholder} value={statusId} onChange={e => { setStatusId(e.target.value); setStatusChecked(false); }} />
              <button style={styles.primaryBtn} onClick={checkStatus}>{t.status.check}</button>
            </div>
            {statusChecked && (
              <div style={{ marginTop: 20 }}>
                {statusResult ? (
                  <div style={{ background: "rgba(127,106,248,0.12)", border: "1px solid rgba(127,106,248,0.24)", borderRadius: 12, padding: 20 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                      <div>
                        <p style={{ color: "#b8b2e4", fontSize: 12, margin: "0 0 4px", textTransform: "uppercase" }}>Project ID</p>
                        <p style={{ color: "#fff", fontSize: 14, fontWeight: 600, margin: 0 }}>{statusResult.id}</p>
                      </div>
                      <div>
                        <p style={{ color: "#b8b2e4", fontSize: 12, margin: "0 0 4px", textTransform: "uppercase" }}>Status</p>
                        <div style={{ background: statusBg[statusResult.status], color: statusColor[statusResult.status], padding: "4px 12px", borderRadius: 20, fontSize: 12, fontWeight: 500, display: "inline-block" }}>
                          {t.admin.statuses[statusResult.status]}
                        </div>
                      </div>
                      <div>
                        <p style={{ color: "#b8b2e4", fontSize: 12, margin: "0 0 4px", textTransform: "uppercase" }}>Name</p>
                        <p style={{ color: "#d0c7ff", fontSize: 14, margin: 0 }}>{statusResult.name}</p>
                      </div>
                      <div>
                        <p style={{ color: "#b8b2e4", fontSize: 12, margin: "0 0 4px", textTransform: "uppercase" }}>Type</p>
                        <p style={{ color: "#d0c7ff", fontSize: 14, margin: 0 }}>{statusResult.type}</p>
                      </div>
                    </div>
                  </div>
